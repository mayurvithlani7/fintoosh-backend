  ...
  try {
    ...
    if (status === 'Approved' && (approval.type === 'move-points' || approval.type === 'points-move')) {
      try {
        ...
        // Verify that pending reservation exists for this point transfer
        if (pendingPoints < approval.amount) {
          return res.status(400).json({
            message: `Insufficient pending points for point transfer approval. Reserved: ${pendingPoints}, Required: ${approval.amount}. The transfer reservation may have expired or been invalidated.`
          });
        }
    ...
    if (status === 'Approved' && approval.type === 'goal-completion') {
      try {
        ...
        if (goal) {
          ...
          // Verify that pending reservation exists for this goal
          if (pendingPoints < target) {
            return res.status(400).json({
              message: `Insufficient pending points for goal approval. Reserved: ${pendingPoints}, Required: ${target}. The goal reservation may have expired or been invalidated.`
            });
          }
